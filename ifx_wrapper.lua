...
local function is_source_file (path)
  ...
  -- Fortran source extensions (fixed and free format, with and without preprocessing).
  -- Uppercase extensions (.F, .F90, etc.) are lowercased before comparison.
  return (ext == ".f")   or (ext == ".f90") or (ext == ".f95") or
         (ext == ".f03") or (ext == ".f08") or (ext == ".for") or
         (ext == ".ftn")
end
  ...
  for i, arg in ipairs(ARGS) do
    if not skip_next_arg then
      local is_unwanted_arg = arg_starts_with(arg, "Fo") or
                              ...
                              arg_starts_with(arg, "module") or -- /module:<dir> for .mod output path
                              is_source_file(arg)
